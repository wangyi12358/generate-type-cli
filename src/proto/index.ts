...
export function getProto2ApiData(options: Options) {
  ...
  const visitRoot = (item: protoJs.Root) => {
    ...
    if (item.filename && apiFile) {
      ...
      if (isType(item)) {
        const namespace = apiFile.apiModules?.[0]?.namespace
        const _interface = typeGenInterface(item as any, namespace);
        if ((item as any).nested) {
          _interface.module = typeGenInterfaceModule(item as any, namespace);
        }
